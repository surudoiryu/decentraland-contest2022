...
//Grass patch
new Model(
  resources.scenery.stone,
  new Transform({ position: new Vector3(8, 0, 8) }),
  _scene
);
new Model(
  resources.scenery.stone,
  new Transform({ position: new Vector3(8, 0, 24) }),
  _scene
);
new Model(
  resources.scenery.stone,
  new Transform({ position: new Vector3(8, 0, 40) }),
  _scene
);

//Floor patch
new Model(
  resources.scenery.floor,
  new Transform({
...
new Model(
  resources.scenery.iceGate,
  new Transform({
    position: new Vector3(6.3, 0, 22.09),
    rotation: new Quaternion(0, 0, 0, 1),
    scale: new Vector3(1, 1, 1),
  }),
  _scene
...
// Infoboard
new Model(
  resources.scenery.board,
  new Transform({
    position: new Vector3(5.34, 0.3, 37.4),
...
new Model(
  ...
  new Transform({
    ...
    rotation: new Quaternion(
      -0.7089106440544128,
      7.500526722880718e-15,
      8.450872002185861e-8,
      -0.7052983641624451
    ),
    scale: new Vector3(
      1.5887510776519775,
...
// Misc objects
const resetButton = new Model(
  resources.models.resetButton,
  new Transform({
    position: new Vector3(12.5, 0.35, 26.25),
    rotation: new Quaternion(0, 0, 0, 1),
    scale: new Vector3(1, 1, 1),
  }),
  _scene
...
const imageBillboardBlack = new Model(
  resources.scenery.billboard,
  new Transform({
    position: new Vector3(8, 7.004123687744141, 19.811948776245117),
    rotation: new Quaternion(
      8.366993570560347e-16,
      -0.7041155099868774,
      8.393710260179432e-8,
      0.7100855708122253
    ),
    scale: new Vector3(
      3.773837089538574,
...
*/
const ball1 = new BowlingBall(
  new Transform({ position: new Vector3(7.92, 1.06, 26.5) }),
  physicsMaterial,
  world,
  _scene
);
const ball2 = new BowlingBall(
  new Transform({ position: new Vector3(7.92, 1.06, 25.88) }),
  physicsMaterial,
  world,
  _scene
);
const ball3 = new BowlingBall(
  new Transform({ position: new Vector3(7.92, 1.06, 25.35) }),
  physicsMaterial,
  world,
  _scene
);
...
*/
const pin1 = new BowlingPin(
  new Transform({ position: new Vector3(9.73, 0.3, 3.18) }),
  physicsMaterial,
  world,
  _scene
);

const pin2 = new BowlingPin(
  new Transform({ position: new Vector3(9.48, 0.3, 2.93) }),
  physicsMaterial,
  world,
  _scene
);
const pin3 = new BowlingPin(
  new Transform({ position: new Vector3(9.98, 0.3, 2.93) }),
  physicsMaterial,
  world,
  _scene
);

const pin4 = new BowlingPin(
  new Transform({ position: new Vector3(9.23, 0.3, 2.68) }),
  physicsMaterial,
  world,
  _scene
);
const pin5 = new BowlingPin(
  new Transform({ position: new Vector3(9.73, 0.3, 2.68) }),
  physicsMaterial,
  world,
  _scene
);
const pin6 = new BowlingPin(
  new Transform({ position: new Vector3(10.23, 0.3, 2.68) }),
  physicsMaterial,
  world,
  _scene
);

const pin7 = new BowlingPin(
  new Transform({ position: new Vector3(8.98, 0.3, 2.43) }),
  physicsMaterial,
  world,
  _scene
);
const pin8 = new BowlingPin(
  new Transform({ position: new Vector3(9.48, 0.3, 2.43) }),
  physicsMaterial,
  world,
  _scene
);
const pin9 = new BowlingPin(
  new Transform({ position: new Vector3(9.98, 0.3, 2.43) }),
  physicsMaterial,
  world,
  _scene
);
const pin10 = new BowlingPin(
  new Transform({ position: new Vector3(10.48, 0.3, 2.43) }),
  physicsMaterial,
  world,
  _scene
);
...
const allyRShape: CANNON.Box = new CANNON.Box(new CANNON.Vec3(4.7, 5, 22.5));
...
const allyLShape: CANNON.Box = new CANNON.Box(new CANNON.Vec3(8.3, 5, 22.5));
...
let isStrike = false;
let isSpare = false;
const powerMeter = new PowerBar(bowlingCanvas, resources.textures.powerMeter);
const swingMeter = new SwingBar(bowlingCanvas, resources.textures.swingMeter);
powerMeter.hideUI();
swingMeter.hideUI();
...
input.subscribe("BUTTON_DOWN", ActionButton.POINTER, false, (e) => {
  for (const ball of balls) {
    if (ball.isActive && !ball.isThrown) {
      if (ballState === 1) {
        ballState = 2;
        const throwDirection = Vector3.Forward().rotate(
  ...
  update(dt: number): void {
    ...
    if (isStrike) {
      const currentPos: number = Number(
        strikeSprite.positionY.toString().slice(0, -2)
      );
    ...
    if (isSpare) {
      const currentPos: number = Number(
        spareSprite.positionY.toString().slice(0, -2)
      );
    ...
    for (let i = 0; i < balls.length; i++) {
      if (!balls[i].isActive) {
        balls[i]
          ...
          .position.copyFrom(balls[i].body.position);
        balls[i]
          .getComponent(Transform)
          .rotation.copyFrom(balls[i].body.quaternion);
      }
    ...
    for (let i = 0; i < pins.length; i++) {
      ...
      pins[i]
        .getComponent(Transform)
        .rotation.copyFrom(pins[i].body.quaternion);
    }
  }
}
