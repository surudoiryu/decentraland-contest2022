...
export class SwingBar {
    ...
    constructor(bowlingCanvas: UICanvas,  texture: Texture) {
        ...
        this.pointer = new UIImage(
            this.container,
            resources.textures.swingPointer
        )
    ...
    update(pb: SwingBar): void {
        let position: number = Number(pb.pointer.positionX.toString().slice(0,-2))
        this.power = position
        if(pb.powerUp) {
    ...
    value(): number {
        let power: number = (((this.power - this.maxLeft) * 100) / (this.maxRight-this.maxLeft) -50 )
        return -power
    }
