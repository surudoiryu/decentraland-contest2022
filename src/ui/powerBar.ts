...
export class PowerBar {
  ...
  constructor(bowlingCanvas: UICanvas, texture: Texture) {
    ...
    this.pointer = new UIImage(this.container, resources.textures.powerPointer);
  ...
  update(pb: PowerBar): void {
    const position: number = Number(
      pb.pointer.positionY.toString().slice(0, -2)
    );
    this.power = position;
    if (pb.powerUp) {
  ...
  value(): number {
    let power: number =
      ((this.power - this.maxLeft) * 100) /
      (this.maxRight - this.maxLeft) /
      100;
